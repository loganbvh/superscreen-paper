...
if __name__ == "__main__":
    ...
    parallel_method = "ray" if (args.num_cpus > 0) else None
    ...
    if args.figures:
        ...
        ax.set_title("Fractional change in polygon flux vs. iteration")
    ...
    pl_flux = df["pl_hull"].values[-1] * squid.ureg("Phi_0")
    ...
    squid.layers["sample_layer"].Lambda = 10 

    if args.sweep_type == "Lambda":
        # Sample effective penetration depth in microns
        sweep_vals = Lambdas = np.logspace(0, 2, args.num_points)
        ...
        sweep_vals = zs = np.linspace(2, 10, args.num_points)
        layer_update_kwargs = [dict(Lambda=None, z0=-z) for z in zs]
        plot_xlabel = "SQUID - sample distance, $z_0$ [$\\mu$m]"
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for path, kwargs in zip(paths, layer_update_kwargs):
            ...
            if args.figures:
                ...
                for ax in axes:
                    _ = squid.plot_polygons(ax=ax, color="w", linewidth=0.5, legend=False)
                fig.suptitle(title)
