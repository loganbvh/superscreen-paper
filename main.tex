...
trapped_flux_solution = sc.solve(
    device=square_ring, circulating_currents=dict(hole="1 mA"),
)[-1]
...
\section{Fluxoid path independence}
\label{appendix:fluxoid}

\SuperScreen allows one to calculate the fluxoid $\Phi^f_S$ for any polygonal region $S$ whose boundary $\partial S$ lies completely within a superconducting film. As described in Section~\ref{section:model:fluxoid}, for a valid solution to Equation~\ref{eq:london} the fluxoid $\Phi^f_S$ should have the following properties:
\begin{enumerate}
...
\begin{subfigure}{\textwidth}
    \centering
    \includegraphics[width=\textwidth]{examples/images/simply_connected_region.png}
    \caption{The fluxoid $\Phi^f_S$ vanishes for simply-connected regions containing no vortices. Current density $|\vec{J}|$ (a) and out-of-plane field $\mu_0H_z$ (b) for the square ring with $\Lambda=1\,\mu\mathrm{m}$ subject to a uniform applied field of 0.1 mT. The ``flux part'' and ``supercurrent part`` of the fluxoid are shown as filled and unfilled bars respectively in (c), with the colors corresponding to the colors of the paths in (a, b).}
    \label{fig:fluxoid-simply}
\end{subfigure}

\begin{subfigure}{\textwidth}
    \centering
    \includegraphics[width=\textwidth]{examples/images/simply_connected_region_with_vortices.png}
    \caption{The fluxoid $\Phi^f_S$}
    \label{fig:fluxoid-simply-vortices}
\end{subfigure}

\begin{subfigure}{\textwidth}
    \centering
    \includegraphics[width=\textwidth]{examples/images/multiply_connected_region.png}
    \caption{Multiply-connected region}
    \label{fig:fluxoid-multiply}
\end{subfigure}
