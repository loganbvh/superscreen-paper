...
\end{tabular}
\caption{Layer stackup for the SQUID susceptometers Section~\ref{section:examples:scanning-squid}. W2 is located at the surface of the chip (deposited last, closest to the sample) and BE deepest below the chip's surface (deposited first). See Ref.~\cite{Kirtley2016-zz} for more details on the designed of the suscpetometers.}
...
\captionof{listing}{Simulating an inhomogeneous superconducting device. Here we modify the superconducting ring with a slit (Code Block~\ref{code:device} to have an effective penetration depth $\Lambda(x, y)$, which increases linearly from $0.1\,\um$ to $1\,\um$ as a function of the angle between the point $(x, y)$ and the $-y$ axis.}
\end{code}

\begin{figure*}[h]
    \centering
    \includegraphics[width=\textwidth]{examples/images/ring_with_slit_inhomogeneous.pdf}
    \caption{The output of Code Block~\ref{code:inhomogeneous}: Screening of a uniform applied field of $10\,\mu\mathrm{T}$ by a ring with a slit in a superconducting film with inhomogeneous effective penetration depth $\Lambda(x, y)$}
