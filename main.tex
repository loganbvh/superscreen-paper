...
\begin{figure*}
    ...
    \caption{The output of Code Block~\ref{code:device}: Meissner screening of a uniform $10\,\mu\mathrm{T}$ out-of-plane field by a ring with inner diameter $1\,\um$, outer diameter $3\,\um$, and effective penetration depth $\Lambda=1\,\um$, interrupted by a slit of width $0.5\,\um$. a) Plot of the boundary of the ring (blue), circular bounding box (orange), and the computational mesh (gray), generated with \inline{Device.plot()}. b) The $z$-component of the magnetic field $\mu_0H_z$ evaluated at the plane of the ring, generated with \inline{Solution.plot_fields()}. c) Current density $\vec{J}$ in the ring, generated with \inline{Solution.plot_currents()}.}
...
\end{tabular}
\caption{Layer stackup for the SQUID susceptometers Section~\ref{section:examples:scanning-squid}. See Ref.~\cite{kirtley_scanning_2016} for more details on the designed of the suscpetometers.}
...
The limitation described above can be seen in the model of the largest SQUID susceptometer described in Section~\ref{section:examples:scanning-squid}, which has a field coil outer radius of $8\,\um$ and a total modeled area of roughly $600\,\um^2$.
